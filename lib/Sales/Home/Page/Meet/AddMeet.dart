...
class _AddMeetState extends State<AddMeet> {
  ...
  Future<void> _addMeet() async {
    ...
    try {
      // Save the data locally for `Activity` screen
      List<String> savedActivities = prefs.getStringList('activities') ?? [];
      savedActivities.add(jsonEncode({
        'date': meetData['date'],
        'type': 'Meet',
        'name': meetData['client_name'],
        'time': '', // Modify as needed
      }));
      await prefs.setStringList('activities', savedActivities);
      ...
      _showSnackBar('Error saving meet: $e');
    }
  }
